...
similarity <- function(user1, user2, alpha = 2, mode = "inverse power"){
  ...
  if(mode == "inverse power"){
    ...
    score = exp(-abs(diff)*alpha)*(cor(as.matrix(df)[1,], as.matrix(df)[2,])+1)
  }
  else if(mode == "linear"){
    score = alpha/(abs(diff)+1)*(cor(as.matrix(df)[1,], as.matrix(df)[2,])+1)
  }
    
  return(score)
}

predict_score <- function(user, df_train, itemID, sd = FALSE, alpha = 1, mode = "inverse power"){
  ...
  relevant_users <- relevant_users %>% 
    mutate(simscore = apply(relevant_users %>% select(c(-n, -mean, -sd)), 1, similarity, user2 = user, alpha = alpha, mode = mode))
...
preds <- test_batch(test, train, sd = TRUE, alpha = 3, mode = "exponential")
