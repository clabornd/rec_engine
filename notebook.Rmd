...
predict_score <- function(users, df_train, sim_matrix = NULL, itemID, sd = TRUE, alpha = 1, mode = "unweighted"){
  ...
  if(is.null(sim_matrix)){
    ...
    relevant_users <- relevant_users %>% 
                      mutate(simscore = apply(relevant_users %>% select(c(-1, -n, -mean, -sd)), 1, similarity, user2 = users[i,], alpha = alpha, mode = mode))
    ...
    if(sd){
        ...
        preds[i] = users[i,]$mean + 
              users[i,]$sd*sum((relevant_users %>% select(itemID[i]))-relevant_users$mean)/
              sum(abs(sims))  
      }
...
preds <- predict_score(test[c(-1,-2)], elite_sparse, sim_matrix = testmatrix, itemID = test$item)  
...
```{r test_nosd, message = FALSE}

indices <- sample(1:nrow(elite_sparse), floor(.1*nrow(elite_sparse)))
  
test <- elite_sparse[indices,]
test <- get_targets(test)
testmatrix <- user_similarity_matrix(elite_sparse, elite_sparse$userID, alpha = 0.1, mode = "exponential")

train <- elite_sparse[-indices,]

train <- train %>%
              mutate(n = apply(train[,-1],1,function(x) sum(!is.na(x))),
              mean = apply(train[,-1], 1, mean, na.rm = TRUE), 
              sd = apply(train[,-1], 1, sd, na.rm = TRUE)) %>%
              filter(sd != 0)

preds <- predict_score(test[c(-1,-2)], elite_sparse, sim_matrix = testmatrix, itemID = test$item)  

sqrt(mean((test$true-preds)^2))
