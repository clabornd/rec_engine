...
similarity <- function(user1, user2, alpha = 2){
...
user <- elite_sparse[1,-1]
test <- elite_sparse[-1, -1]

predict_score(user, test, "11")
...
for(i in 1:nrow(test)){
    ...
    test[i,]$true <- unlist(test[i,index][[1]]) 
    test[i,]$item <- colnames(test[,index])
    
    foo <- test[i, names(test) != c("true", "item")]
    foo[test[i,]$item] <- NA
    
    test[i,]$pred <- predict_score(foo, train, test[i,]$item) 
