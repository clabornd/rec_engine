...
predict_score <- function(user, df_train, itemID, sd = FALSE, alpha = 1, mode = "unweighted"){
...
user_similarity <- function(usermatrix, alpha = 1, mode = "unweighted"){
...
simmat <- user_similarity(elite_sparse[1:100,-1])
  ...
  if(mode = "random"){
    for(i in nrow(df)){
      #get a random column which is not empty
        ...
        true[i] <- df[i,index][[1]] 
        item[i] <- colnames(df[,index])
        
        df[i, item[i]] <- NA
    }
    
    return(cbind(item, true,df))
