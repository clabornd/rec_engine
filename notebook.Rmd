---
title: "PCA and User-User Collaborative for Boardgame Ratings Data"
output:
...
```{r, fig.cap = "**Scatterplots of mean rating (red) and standard deviation (blue) against number of reviews**"}
...
```{r, fig.cap = "**Pairs of the first 7 principal components**"}
...
We will now attempt to build a recommendation engine based on the full user data.  The process will use a basic User-User collaborative filtering scheme with some experimental adjustments to predict scores.

The similarity between two users will be determined by calculating a scaled Pearson's Correlation coefficient:

$$s(u,v)=\frac{1}{\vert{D_{u,v}(N)\vert}^\alpha}{\huge[}\frac{\sum_{i\in I_{common}}(r_{u,i}-\overline{r}_{u,i})(r_{v,i}-\overline{r}_{v,i})}{\sqrt{\sum_{i\in I_{common}}(r_{u,i}-\overline{r}_{u,i})^2}\sqrt{\sum_{i\in I_{common}}(r_{v,i}-\overline{r}_{v,i})^2}}+1{\huge]}$$
...
To see how our engine did, we can look at the correlation between the true and predicted values, as well as the root mean squared error of our predictions:

```{r, echo = TRUE}
cor(preds$predicted, preds$true)
...
The correlation isn't particularly strong, but hey at least its positive!  On average, our predictions are off by one and a half points, not particularly impressive.  To improve performance, we can scale the difference for each user score from their mean by the standard deviation of that users ratings:
...
predict_score <- function(user, df_train, itemID){
  ...
  predict = user$mean + user$sd*sum(relevant_users$simscore*((relevant_users %>% select(itemID))-relevant_users$mean)/relevant_users$sd)/sum(abs(relevant_users$simscore))
...
Where $\sigma_u$ and $\sigma_{u'}$ indicate the standard deviation of the scores for the user in question and an arbitrary *other* user respectively
