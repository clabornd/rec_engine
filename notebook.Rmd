...
similarity <- function(user1, user2, alpha = 10){
...
predict_score <- function(user, df_train, itemID){
  ...
  relevant_users <- df_train %>% filter(!is.na(!!itemID_quo))
  ...
  relevant_users <- relevant_users %>% mutate(simscore = apply(relevant_users %>% select(c(-n, -mean, -sd)), 1, similarity, user))
  
  print(relevant_users$simscore*((relevant_users %>% select(!!itemID_quo))-relevant_users$mean))
  
  predict = user$mean + sum(relevant_users$simscore*((relevant_users %>% select(!!itemID_quo))-relevant_users$mean))/sum(abs(relevant_users$simscore))
  
  return(predict)
}

predict_score(user[,-1], train[,-1], `188`)


hi <- predict_score(user[,-1], train[,-1], `116`)

mean(train[1,])


user <- elite_sparse[1,-1] %>% mutate(`45`=NA)
train <- elite_sparse[-1,-1]
